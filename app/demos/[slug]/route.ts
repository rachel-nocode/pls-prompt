...
export async function GET(_request:Request,{params}:{params:Promise<{slug:string}>}) {
  ...
  if(Object.hasOwn(demoBundles,slug))return new Response(demoBundles[slug],{headers:demoHeaders});
  ...
  if(!(actor?.isCreator&&object.customMetadata?.owner===actor.id)&&!await(await repository()).canReadRecipeMedia(`/demos/${slug}`))return new Response("Demo unavailable",{status:404,headers:demoHeaders});
