...
export async function POST(request: Request) {
  try {
    ...
    const recordId = data.kind === "recipe" ? await store.editRecipe(actor, data.id, data.version, data.input, data.publish, data.showcase) : data.kind === "prompt" ? await store.editPrompt(actor, data.id, data.version, data.input) : await store.editLesson(actor, data.id, data.version, data.input);
