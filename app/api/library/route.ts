...
const item = z.object({ title: z.string().trim().min(1).max(140), promptText: z.string().max(30000), notes: z.string().max(6000), tags: z.array(z.string().trim().min(1).max(40)).max(12) });
const schema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("save"), promptId: id }),
...
export async function POST(request: Request) {
  try {
    ...
    switch (data.action) {
      case "save": result = await store.savePrompt(actor, data.promptId); break;
