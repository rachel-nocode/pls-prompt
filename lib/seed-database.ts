...
import { promptExercises } from "./prompt-exercises.ts";
...
export async function seedDatabase(database: SqlDatabase) {
  ...
  if (!await database.prepare("SELECT id FROM content_revisions WHERE id=?").bind(revision).first()) {
  ...
  const exerciseRevision = "prompt-completion-exercises-v1";
  if (!await database.prepare("SELECT id FROM content_revisions WHERE id=?").bind(exerciseRevision).first()) {
    const time = new Date().toISOString();
    const statements = beginnerLessons.map(lesson => database.prepare(
      "UPDATE lessons SET check_data=?,version=version+1,updated_at=? WHERE id=? AND check_data=? AND NOT EXISTS(SELECT 1 FROM content_revisions WHERE id=?)"
    ).bind(JSON.stringify(promptExercises[lesson.id]), time, lesson.id, JSON.stringify(lesson.check), exerciseRevision));
    statements.push(database.prepare("INSERT OR IGNORE INTO content_revisions(id,applied_at) VALUES (?,?)").bind(exerciseRevision, time));
    await database.batch(statements);
  }
}
