...
import type { QuickCheck } from "@/lib/library-types";
...
const emptyCheck: QuickCheck = { question: "", options: [{ id: "a", text: "" }, { id: "b", text: "" }, { id: "c", text: "" }], correct: "a", explanation: "" };
  ...
  return <section className="work-page"><div className="work-heading"><div><span className="mono-label">CREATOR WORKSPACE</span><h1>Make the next<br />small lesson.</h1><p>Write simply. Reward learners with something worth building.</p></div><Button onClick={() => load()}>New {kind === "prompt" ? "recipe" : "lesson"}</Button></div>
    ...
    <div className="studio-layout"><aside className="studio-list">{records.map(item => <button className={"library-entry " + (selected === item.id ? "selected" : "")} key={item.id} onClick={() => load(item)}><span className="mono-label">{kind === "prompt" ? str(item.status) : item.published ? "PUBLISHED" : "DRAFT"}</span><strong>{item.title}</strong><small>Version {item.version}</small></button>)}</aside>
      <form className="studio-editor" onSubmit={event => { event.preventDefault(); void save(); }}>
        ...
        <p className="field-help">{kind === "prompt" ? "Include the outcome, features, design, data, and checks. Give learners useful defaults so they can paste it straight into their builder." : "Aim for three short steps and one quick check. Explain one idea in plain language."}</p>
        ...
        : <><div className="form-grid"><label>Minutes<Input type="number" min={1} max={5} value={form.minutes} onChange={event => edit("minutes", Number(event.target.value))} /></label><label>Order<Input type="number" min={1} max={100} value={form.position} onChange={event => edit("position", Number(event.target.value))} /></label><label>Previous lesson<NativeSelect value={form.prerequisiteId} onChange={event => edit("prerequisiteId", event.target.value)}><option value="">Start of path</option>{data.lessons.filter(item => item.id !== selected).map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</NativeSelect></label><label>Completion reward<NativeSelect value={form.rewardPromptId} onChange={event => edit("rewardPromptId", event.target.value)}><option value="">Choose a recipe</option>{data.prompts.filter(item => item.status === "published" && item.prompt_text).map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</NativeSelect></label></div>
          <fieldset className="author-check"><legend>Quick check</legend><label>Question<Input maxLength={350} value={form.check.question} onChange={event => edit("check", { ...form.check, question: event.target.value })} /></label>{form.check.options.map((option, index) => <label key={option.id}>Answer {index + 1}<Input maxLength={350} value={option.text} onChange={event => edit("check", { ...form.check, options: form.check.options.map((current, optionIndex) => optionIndex === index ? { ...current, text: event.target.value } : current) })} /></label>)}<label>Correct answer<NativeSelect value={form.check.correct} onChange={event => edit("check", { ...form.check, correct: event.target.value })}>{form.check.options.map((option, index) => <option key={option.id} value={option.id}>Answer {index + 1}</option>)}</NativeSelect></label><label>Explain why<Textarea maxLength={600} value={form.check.explanation} onChange={event => edit("check", { ...form.check, explanation: event.target.value })} /></label></fieldset></>}
        <details className="reward-preview"><summary>Preview text</summary><h2>{form.title || "Untitled"}</h2><p>{form.summary}</p><pre>{form.body}</pre>{kind === "lesson" && <><h3>{form.check.question}</h3><ul>{form.check.options.map(option => <li key={option.id}>{option.text}</li>)}</ul></>}</details>
